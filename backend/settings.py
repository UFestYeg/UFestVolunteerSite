...
TEMPLATES = [
    {
        ...
        # `build` is listed first so the real, asset-referencing SPA shell
        # generated by the frontend build wins when present (production). The
        # checked-in `templates/index.html` is a fallback used only when the
        # build output is absent (e.g. CI backend tests), so the SPA-fallback
        # view still resolves a template instead of raising TemplateDoesNotExist.
